...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen flex flex-col pb-24 relative pt-6 w-full lg:p-20 mx-auto bg-[#88f6c5]">
      {/* <AnimatedBackground /> */}
      <Bannner/>
      <Actions />
      ...
      <section className="px-4 lg:px-6 mb-8">
        ...
        {error && !loading && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-gray-800 mb-2">Unable to Load Games</h3>
            <p className="text-black/70 mb-4">{error}</p>
            <button 
              onClick={() => window.location.reload()}
              className="px-6 py-2 bg-black/20 text-white rounded-lg hover:bg-white/30 transition-colors duration-200"
            >
              Try Again
            </button>
          </div>
        ...
        {!loading && !error && games.length > 0 && (
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 2xl:grid-cols-8 gap-3 md:gap-4 lg:gap-5 bg-surface p-2 border-dashed border-gray-300 rounded-xl">
            {games.map((game, idx) => (
